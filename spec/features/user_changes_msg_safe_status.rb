feature 'User changes msg safe status' do
  ...
  scenario 'finds the safe link' do
    visit_unsafe_path
    page.find_by_id("message_#{message.id}").find("a.unsafe")
  end

  scenario 'clicking on safe link' do
    visit_unsafe_path
    page.find_by_id("message_#{troll_message.id}").find("a.safe").click
  ...
  scenario 'finds the unsafe link' do
    visit_unsafe_path
    page.find_by_id("message_#{troll_message.id}").find("a.unsafe")
  end

  scenario 'cliking on unsafe link' do
    visit_unsafe_path
    page.find_by_id("message_#{message.id}").find("a.unsafe").click
    message.reload
